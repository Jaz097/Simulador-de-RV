...
public class ParacaidismoMedio : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (isFalling && !isParachuteOpen && !hasCollided)
        {
            ...
            if (eventoAgarre.esAbierto && altitud >= 750f)
            {
                OpenParachute();
            ...
            else if (eventoAgarre.esAbierto && altitud <= 750f)
            {
    ...
    void OnTriggerEnter(Collider other)
    {
        if (other.CompareTag("Terrain")) // Asegúrate de que el terreno tenga la etiqueta "Terrain"
        {
            ...
            if (isParachuteOpen && ObtenerAltura_boton >= 750f)
            {
