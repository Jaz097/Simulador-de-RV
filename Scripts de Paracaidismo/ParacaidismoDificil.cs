...
public class ParacaidismoDificil : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (isFalling && !isParachuteOpen && !hasCollided)
        {
            ...
            if (eventoAgarre.esAbierto && altitud >= 800f)
            {
                OpenParachute();
            ...
            else if (eventoAgarre.esAbierto && altitud <= 800f)
            {
    ...
    private bool PuedeAvanzarInstruccion()
    {
        if (CheckButtonPress(XRNode.RightHand, CommonUsages.secondaryButton) &&
            (Time.time - tiempoUltimaPresion) > delayPresion && contadorReinicios == 0)
        {
    ...
    void OnTriggerEnter(Collider other)
    {
        if (other.CompareTag("Terrain")) // Asegúrate de que el terreno tenga la etiqueta "Terrain"
        {
            ...
            if (isParachuteOpen && ObtenerAltura_boton >= 800f)
            {
